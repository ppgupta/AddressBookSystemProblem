...
public class AddressBookPerson {
	...
	// function to add new contacts to the address book
		public boolean addNewContact(String firstName, String lastName,String address, String city, String state, int phoneNumber,
			...
			System.out.println("Contact details has been added :");
		...
		public void deleteContact() {
			...
			Scanner sc = new Scanner(System.in);
			String name = sc.nextLine();
			for (int count = 0; count < this.bookList.size(); count++) {

				if (this.bookList.get(count).getFirstName().equalsIgnoreCase(name)) {
					this.bookList.remove(this.bookList.get(count));
		...
		public void getNumberofContactPerson(String personName) {
			// first checking whether a person  exist in the address book or not
		...
		public void writeAddressBookData(IOAddressBookService ioService) {
			if(ioService.equals(IOAddressBookService.FILE_IO)) {
				new AddressBookFileIO().writeData(this.bookList,this.bookName);
		...
		public void readAddressBookData(IOAddressBookService ioService) {
			if(ioService.equals(IOAddressBookService.FILE_IO)) {
				List<ContactDetails> contactList = new AddressBookFileIO().readData(this.bookName);
		...
		public void printData(IOAddressBookService ioService) {
			if(ioService.equals(IOAddressBookService.FILE_IO)) {
				new AddressBookFileIO().printData(this.bookName);
			}
		}
